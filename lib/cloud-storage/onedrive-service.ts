...
export async function listFiles(
  ...
  let response;
  try {
    // Fetch both files and folders (no filter)
    console.log(`[OneDrive Service] Fetching files and folders`);
    response = await request
      .top(100)
      .get();
    console.log(`[OneDrive Service] Successfully fetched, found ${response.value?.length || 0} items`);
  } catch (error: any) {
    console.error(`[OneDrive Service] Failed to fetch items:`, {
      error: error.message,
      code: error.code,
  ...
  const files: CloudFile[] = (response.value || []).map((item: any) => {
    const isFolder = item.folder !== null && item.folder !== undefined;
    return {
      id: item.id,
      name: item.name,
      size: item.size || 0,
      mimeType: item.file?.mimeType || (isFolder ? 'application/vnd.microsoft.folder' : 'application/octet-stream'),
      provider: siteId ? 'sharepoint' : 'onedrive',
      url: item.webUrl,
